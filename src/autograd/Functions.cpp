...
#undef INSTANTIATE_FUNCTION
      ...
      Variable negate(const Variable &input)
        ...
        Variable convolve2(const Variable &input, const Variable &weights,
                                                           int sx, int sy,
                                                           int px, int py,
                                                           int dx, int dy)
        {
            array result = af::convolve2(input.array(), weights.array(), dim4(sx, sy), dim4(px, py), dim4(dx, dy));

            auto grad_func = [result, sx, sy, px, py, dx, dy](std::vector<Variable> &inputs, const Variable &grad_output) {

                array wGrad = convolve2Gradient(grad_output.array(), inputs[0].array(), inputs[1].array(),
                                                result, dim4(sx, sy), dim4(px, py), dim4(dx, dy), AF_CONV_GRADIENT_FILTER);
                array iGrad = convolve2Gradient(grad_output.array(), inputs[0].array(), inputs[1].array(),
                                                result, dim4(sx, sy), dim4(px, py), dim4(dx, dy), AF_CONV_GRADIENT_DATA);

                inputs[0].addGrad(Variable(iGrad, false));
                inputs[1].addGrad(Variable(wGrad, false));
            };
            return Variable(result, {input, weights}, grad_func);
